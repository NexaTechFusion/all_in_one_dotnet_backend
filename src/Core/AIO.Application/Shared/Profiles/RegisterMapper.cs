...
public class RegisterMapper : Profile
{
    ...
    private void ApplyMappingProfiles(Assembly assembly)
    {
        var types = assembly.GetExportedTypes().Where(t => t.GetInterfaces().Any(i =>
            ...
            .ToList();
        List<Type> reverseTypes = assembly.GetExportedTypes().Where(t => t.GetInterfaces().Any(i =>
        ...
        foreach (Type type in reverseTypes)
        {
            ...
            MethodInfo methodInfo = type.GetMethod("Map") //get the map method directly by the class
                                    ?? type.GetInterface("IMapTo`1")
                                        .GetMethod("Map"); //if null get the interface implementation
